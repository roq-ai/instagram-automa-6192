...
import { getPricingById } from 'apiSdk/pricings';
import { PricingInterface } from 'interfaces/pricing';

function PricingViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<PricingInterface>(
    () => (id ? `/pricings/${id}` : null),
    () =>
      getPricingById(id, {
        relations: ['user'],
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('pricing', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/pricings/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                <FormListItem label="Package Name" text={data?.package_name} />
