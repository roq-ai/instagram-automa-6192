...
import { getReferrals } from 'apiSdk/referrals';
...
import { BlogListPage as ShowCaseList } from 'pages/blogs';

function DashboardPage() {
  const { session } = useSession();

  const referralStat = useSWR<PaginatedInterface<ReferralInterface>>(
    () => `/referrals`,
    () => getReferrals({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Referrals',
      result: referralStat?.data?.totalCount,
      isLoading: referralStat.isLoading,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`blogs`} passHref legacyBehavior>
                  <Button
