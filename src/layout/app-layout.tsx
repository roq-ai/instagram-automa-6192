...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['team-admin', 'team-member'];
  ...
  const { data, error, isLoading } = useSWR<AccountInterface[]>(
    ...
    () => getAccounts({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    { name: 'Pricings', path: '/pricings', entity: 'pricing', service: AccessServiceEnum.PROJECT, icon: FiTag },
    { name: 'Referrals', path: '/referrals', entity: 'referral', service: AccessServiceEnum.PROJECT, icon: FiLink },
    {
      name: 'Subscriptions',
      path: '/subscriptions',
      entity: 'subscription',
